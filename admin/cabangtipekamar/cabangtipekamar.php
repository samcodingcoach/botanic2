...
// Fetch data from API
$apiUrl = 'http://localhost/botanic/api/cabangtipekamar/list.php';
...
// Fetch tipe kamar list for dropdown
$tipeKamarApiUrl = 'http://localhost/botanic/api/tipe_kamar/list.php';
$tipeKamarApiResponse = file_get_contents($tipeKamarApiUrl);
$tipeKamarApiData = json_decode($tipeKamarApiResponse, true);
$tipeKamarList = [];
if ($tipeKamarApiData && $tipeKamarApiData['success']) {
    $tipeKamarList = $tipeKamarApiData['data'];
}
?>
<!DOCTYPE html>
<html lang="id">
...
<body class="bg-background-light dark:bg-background-dark text-slate-900 dark:text-slate-100 font-display">
                ...
                <div class="flex items-center gap-4">
                    <div class="relative hidden sm:block">
                        ...
                        <input id="searchInput"
                            ...
                            placeholder="Cari cabang tipe..." type="text" onkeyup="filterData()" />
                    </div>
    ...
    <!-- Add Cabang Tipe Modal -->
    <div class="fixed inset-0 z-50 flex items-center justify-center hidden" id="add-cabangtipe-modal">
        <!-- Backdrop -->
        ...
        <div
            ...
            <div class="overflow-y-auto px-6 py-4 space-y-4 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-600">
                <form id="formTambahCabangTipe" class="space-y-4" enctype="multipart/form-data">
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <div class="col-span-2">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Tipe
                                Kamar <span class="text-red-500">*</span></label>
                            <select id="id_tipe" name="id_tipe"
                                class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none text-sm"
                                required>
                                <option value="">-- Pilih Tipe Kamar --</option>
                                <?php foreach ($tipeKamarList as $tipe): ?>
                        ...
                        <div class="col-span-2">
                            ...
                            <textarea id="keterangan" name="keterangan"
                                ...
                                placeholder="Deskripsi akomodasi..."></textarea>
                        </div>
                        <div class="col-span-2">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Link
                                YouTube</label>
                            <input id="link_youtube" name="link_youtube"
                                class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none text-sm"
    ...
    <!-- Edit Cabang Tipe Modal -->
    <div class="fixed inset-0 z-50 flex items-center justify-center hidden" id="edit-cabangtipe-modal">
        <!-- Backdrop -->
        <div class="absolute inset-0 bg-slate-900/60 backdrop-blur-sm"></div>
        <!-- Modal Content -->
        <div
            ...
            <!-- Header (Fixed) -->
            <div class="px-6 py-4 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between flex-shrink-0">
                <h3 class="text-lg font-bold text-slate-900 dark:text-white">Edit Cabang Tipe</h3>
            ...
            <!-- Scrollable Content -->
            <div class="overflow-y-auto px-6 py-4 space-y-4 scrollbar-thin scrollbar-thumb-slate-300 dark:scrollbar-thumb-slate-600">
                <form id="formEditCabangTipe" class="space-y-4" enctype="multipart/form-data">
                    <input type="hidden" id="edit_id_akomodasi" name="id_akomodasi" />
                    <div class="grid grid-cols-2 gap-4">
                        <div class="col-span-2">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Cabang
                                <span class="text-red-500">*</span></label>
                        ...
                        <div class="col-span-2">
                            <label class="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Link
                                YouTube</label>
                            <input id="edit_link_youtube" name="link_youtube"
                                class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none text-sm"
                                placeholder="https://youtube.com/watch?v=..." type="url" />
                        </div>
                    </div>
    ...
    <script>
        ...
        function confirmSave() {
            return new Promise((resolve) => {
                const confirmed = confirm('Apakah Anda yakin ingin menyimpan data cabang tipe ini?');
                resolve(confirmed);
            });
        }

        // Modal functions
        const modal = document.getElementById('add-cabangtipe-modal');
        const btnTambah = document.getElementById('btnTambahCabangTipe');
        const btnClose = document.querySelectorAll('.btn-close-modal');
        const btnSimpan = document.getElementById('btnSimpan');
        const formTambah = document.getElementById('formTambahCabangTipe');
        ...
        if (btnSimpan) {
            btnSimpan.addEventListener('click', async function() {
                // Validate form
                const requiredFields = ['id_cabang', 'id_tipe'];
                let isValid = true;
                let emptyFields = [];
                ...
                // Send to API
                fetch('../../api/cabangtipekamar/new.php', {
                    method: 'POST',
                    body: formData
                })
                ...
                .then(data => {
                    if (data.success) {
                        showToast('Data cabang tipe berhasil disimpan!', 'success');
                        closeModal();
        ...
        const editModal = document.getElementById('edit-cabangtipe-modal');
        const btnCloseEdit = document.querySelectorAll('.btn-close-edit-modal');
        const btnUpdate = document.getElementById('btnUpdate');
        const formEdit = document.getElementById('formEditCabangTipe');

        function openEditModal(idAkomodasi, idCabang, namaCabang, idTipe, namaTipe, keterangan, linkYoutube) {
            document.getElementById('edit_id_akomodasi').value = idAkomodasi;
            document.getElementById('edit_id_cabang').value = idCabang;
            document.getElementById('edit_id_tipe').value = idTipe;
            document.getElementById('edit_keterangan').value = keterangan;
            document.getElementById('edit_link_youtube').value = linkYoutube;
        ...
        document.addEventListener('click', function(e) {
            if (e.target.closest('.btn-edit-cabangtipe')) {
                const btn = e.target.closest('.btn-edit-cabangtipe');
                openEditModal(
                    btn.dataset.idakomodasi,
                    btn.dataset.idcabang,
                    btn.dataset.namacabang,
                    btn.dataset.idtipe,
                    btn.dataset.namatipe,
                    btn.dataset.keterangan,
                    btn.dataset.linkyoutube
                );
            }
            // Handle edit button clicks (mobile)
            if (e.target.closest('.btn-edit-cabangtipe-mobile')) {
                const btn = e.target.closest('.btn-edit-cabangtipe-mobile');
                openEditModal(
                    btn.dataset.idakomodasi,
                    btn.dataset.idcabang,
                    btn.dataset.namacabang,
        ...
        if (btnUpdate) {
            btnUpdate.addEventListener('click', async function() {
                // Validate form
                const requiredFields = ['edit_id_cabang', 'edit_id_tipe'];
                let isValid = true;
                let emptyFields = [];
                ...
                const confirmed = confirm('Apakah Anda yakin ingin mengupdate data cabang tipe ini?');
                if (!confirmed) return;
                ...
                // Prepare JSON data
                const jsonData = {
                    id_akomodasi: document.getElementById('edit_id_akomodasi').value,
                    id_cabang: document.getElementById('edit_id_cabang').value,
                    id_tipe: document.getElementById('edit_id_tipe').value,
                    keterangan: document.getElementById('edit_keterangan').value,
                    link_youtube: document.getElementById('edit_link_youtube').value
                };

                // Send to API
                fetch('../../api/cabangtipekamar/update.php', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        showToast('Data cabang tipe berhasil diupdate!', 'success');
                        closeEditModal();
                        // Refresh page after short delay
                        setTimeout(() => {
                            location.reload();
        ...
        // Data from PHP
        const allData = <?php echo json_encode($cabangTipeList); ?>;
        ...
        function filterData() {
            ...
            filteredData = allData.filter(item => {
                return (
                    ...
                    (item.keterangan_akomodasi && item.keterangan_akomodasi.toLowerCase().includes(searchTerm))
                );
            });
            currentPage = 1;
        ...
        function renderTable() {
            ...
            tableBody.innerHTML = pageData.map(item => `
                <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                    ...
                    <td class="px-6 py-4">
                        <div class="text-sm text-slate-600 dark:text-slate-400 max-w-xs truncate">${item.keterangan_akomodasi || '-'}</div>
                    </td>
                    <td class="px-6 py-4">
                        ${item.link_youtube ? `
                    ...
                    <td class="px-6 py-4 text-right space-x-2">
                        <button class="p-1.5 text-slate-400 hover:text-primary transition-colors btn-edit-cabangtipe"
                            data-idakomodasi="${item.id_akomodasi}"
                            data-idcabang="${item.id_cabang}"
                            data-namacabang="${item.nama_cabang || ''}"
                            data-idtipe="${item.id_tipe}"
                            data-namatipe="${item.nama_tipe || ''}"
                            data-keterangan="${item.keterangan_akomodasi || ''}"
                            data-linkyoutube="${item.link_youtube || ''}"
                            title="Edit">
                            <span class="material-symbols-outlined text-xl">edit_square</span>
                        </button>
                        <button class="p-1.5 text-slate-400 hover:text-red-500 transition-colors btn-delete-cabangtipe"
                            data-idakomodasi="${item.id_akomodasi}"
                            data-namacabang="${item.nama_cabang || ''}"
                            data-namatipe="${item.nama_tipe || ''}"
                            title="Hapus">
                ...
                <div class="p-4 space-y-3">
                    <div class="flex items-start gap-3">
                        ...
                        <div class="flex-1 min-w-0">
                            <div class="font-semibold text-slate-900 dark:text-white">${item.nama_cabang || '-'}</div>
                            <div class="text-xs text-slate-500">${item.nama_tipe || '-'}</div>
                        </div>
                        <div class="flex gap-1">
                            <button class="p-2 text-slate-400 hover:text-primary transition-colors btn-edit-cabangtipe-mobile"
                                data-idakomodasi="${item.id_akomodasi}"
                                data-idcabang="${item.id_cabang}"
                                data-namacabang="${item.nama_cabang || ''}"
                                data-idtipe="${item.id_tipe}"
                            ...
                            <button class="p-2 text-slate-400 hover:text-red-500 transition-colors btn-delete-cabangtipe-mobile"
                                data-idakomodasi="${item.id_akomodasi}"
                                data-namacabang="${item.nama_cabang || ''}"
                                data-namatipe="${item.nama_tipe || ''}"
                                title="Hapus">
        ...
        document.addEventListener('click', function(e) {
            const deleteBtn = e.target.closest('.btn-delete-cabangtipe, .btn-delete-cabangtipe-mobile');
            if (deleteBtn) {
                const id = deleteBtn.dataset.idakomodasi;
                const namaCabang = deleteBtn.dataset.namacabang;
                const namaTipe = deleteBtn.dataset.namatipe;

                if (confirm(`Apakah Anda yakin ingin menghapus "${namaCabang} - ${namaTipe}"?`)) {
                    fetch('../../api/cabangtipekamar/delete.php', {
                        method: 'POST',
                        headers: {
                            'Content-Type': 'application/json'
                        },
                        body: JSON.stringify({ id_akomodasi: parseInt(id) })
                    })
                    .then(response => response.json())
                    .then(data => {
                        if (data.success) {
                            showToast('Data cabang tipe berhasil dihapus!', 'success');
                            setTimeout(() => {
                                location.reload();
                            }, 1500);
                        } else {
