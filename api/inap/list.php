...
// Get id_guest from query parameter (optional for admin, required for public)
$id_guest = null;
$isAdmin = false;
...
// Check if this is an admin request (id_guest = 'all' or not provided)
if (!isset($_GET['id_guest']) || $_GET['id_guest'] === 'all') {
    $isAdmin = true;
}

try {
    if ($isAdmin) {
        // Admin query - show all reservations
        $query = "SELECT
            inap.id_inap,
            inap.id_cabang,
            cabang.nama_cabang,
