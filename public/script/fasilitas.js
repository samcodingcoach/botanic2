...
function createFacilityCard(facility, index) {
  ...
  card.className =
    "facility-card bg-white dark:bg-slate-900 rounded-xl overflow-hidden shadow-sm border border-slate-100 dark:border-slate-800 mx-4";
...
window.filterFacilities = function (searchTerm) {
  ...
  cards.forEach((card) => {
    ...
    const matchesSearch =
      namaFasilitas.includes(searchLower) || deskripsi.includes(searchLower);
    ...
    if (matchesSearch) {
      ...
      const facilityName =
        card.querySelector("h3")?.textContent?.trim() || "Unknown";
      ...
      if (
        namaFasilitasEl &&
        namaFasilitasEl.textContent.toLowerCase().includes(searchLower)
      ) {
        namaFasilitasEl.innerHTML = highlightText(
          namaFasilitasEl.textContent,
          searchTerm,
        );
      }
      if (
        deskripsiEl &&
        deskripsiEl.textContent.toLowerCase().includes(searchLower)
      ) {
        deskripsiEl.innerHTML = highlightText(
  ...
  if (visibleCount === 0) {
    ...
    newNoResultsMsg.className =
      "no-results-message flex flex-col items-center justify-center py-12 mx-4";
    ...
    searchInfoEl.className =
      "search-info flex items-center justify-between bg-slate-100 dark:bg-slate-800 rounded-lg px-4 py-3 mb-4 mx-4";
...
window.toggleReadMore = function (button) {
  ...
  if (isExpanded) {
    ...
    const truncatedText =
      fullText.trim().split(/\s+/).slice(0, 30).join(" ") + "...";
